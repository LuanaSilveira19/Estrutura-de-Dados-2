...
void mostra_PreOrd(arvore *t)
{
    ...
    if(!testa_vazia(t))
    {
        cout << "[Mat: "<<t->info.matricula<<", Nome: "<<t->info.nome<<", Méd: "<<t->info.media<<", Freq: "<<t->info.frequencia<<"]";
        mostra_PreOrd(t->sae);
        mostra_PreOrd(t->sad);
    }
    cout << ">";
...
arvore* remover(arvore *t, Alunos num)//com essa função diferente da do professor funcionou certo
{
